"""
URL configuration for Dunimatrix project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path,include
from users import urls as user_urls
from student import urls as student_urls
from teacher import urls as teacher_urls
from section import urls as section_urls
from year import urls as year_urls 
from faculty import urls as faculty_urls
from module import urls as module_urls
from django.conf import settings
from django.conf.urls.static import static
urlpatterns = [
    path('superadmin/', admin.site.urls),
    path('admin/',include(user_urls)),
    path('student/',include(student_urls)),
    path('teacher/',include(teacher_urls)),
    path('section/',include(section_urls)),
    path('faculty/',include(faculty_urls)),
    path('year/',include(year_urls)),
    path('module/',include(module_urls))
]+ static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)



